...
\usepackage{graphicx}
\usepackage{caption}
\usepackage{parskip} % PAQUETE PARA ESPACIADO ENTRE PÁRRAFOS
\usepackage{fancyhdr} % <-- NUEVO PAQUETE PARA PIE DE PÁGINA
...
\onehalfspacing
% \pagestyle{plain} % <-- LÍNEA ORIGINAL ELIMINADA

% --- NUEVA CONFIGURACIÓN DEL PIE DE PÁGINA ---
\pagestyle{fancy} % Activa el estilo fancy
\fancyhf{} % Borra todas las cabeceras y pies de página actuales
\renewcommand{\headrulewidth}{0pt} % Elimina la línea de la cabecera
\lfoot{Todos los derechos reservados} % Tu texto a la izquierda
\rfoot{\thepage} % El número de página a la derecha
