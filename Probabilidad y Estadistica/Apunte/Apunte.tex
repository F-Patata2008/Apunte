...
\fancyfoot[C]{\footnotesize Apunte por Felipe Colli Olea -- Licencia CC BY-NC-ND 4.0} 
...
\section[23/04 - Demostraciones sobre el Valor de la Desviación Estándar]{23/04 - Demostraciones sobre el Valor de la Desviación Estándar}
\subsection[Demostraciones de Propiedades Relacionadas con el Valor de sigma]{Demostraciones de Propiedades Relacionadas con el Valor de $\sigma$}
Recordar que $\sigma \ge 0$ por definición (es una raíz cuadrada positiva o cero).

\subsubsection[Propiedad 6: sigma-cuadrado = sigma si y solo si sigma=0 o sigma=1]{Propiedad 6: \texorpdfstring{$\sigma^2 = \sigma \iff \sigma=0 \vee \sigma=1$}{sigma-cuadrado = sigma <=> sigma=0 v sigma=1}}
...
\subsubsection[Propiedad 7: sigma-cuadrado < sigma si y solo si 0 < sigma < 1]{Propiedad 7: \texorpdfstring{$\sigma^2 < \sigma \iff 0 < \sigma < 1$}{sigma-cuadrado < sigma <=> 0 < sigma < 1}}
...
\subsubsection[Propiedad 8: sigma-cuadrado > sigma si y solo si sigma > 1]{Propiedad 8: \texorpdfstring{$\sigma^2 > \sigma \iff \sigma > 1$}{sigma-cuadrado > sigma <=> sigma > 1}}
Partimos de la desigualdad:
\[\sigma^2 > \sigma \]
Reordenamos:
\[\sigma^2-\sigma > 0 \]
...
\subsubsection{Permutación simple}
Corresponde a la manera en que se pueden ordenar $n$ objetos.
\begin{minipage}{0.35\textwidth}
	\centering
	\[ P_n = n! \]
\end{minipage}
...
Si tienes $n$ elementos y un elemento se repite $r_1$ veces, otro $r_2$ veces y así sucesivamente, entonces se pueden ordenar:
\[ P_{r_1, r_2, \dots}^n = \frac{n!}{r_1! \cdot r_2! \cdot \dots} \]

\subsubsection{Permutación Circular}
Si tenemos $n$ elementos, los podemos ordenar de:
\[ PC_n = \frac{n!}{n} = (n-1)! \]
...
Corresponde a la cantidad de grupos (o subconjuntos) de $k$ elementos de un total de $n$ elementos.
